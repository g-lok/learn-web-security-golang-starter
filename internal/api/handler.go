...
func (handler *Handler) WarehouseOrders(responseWriter http.ResponseWriter, request *http.Request) {
	...
	findApiKey, found, err := handler.apiStore.FindKey(request.Context(), apiKey)
	if err != nil {
		handler.internalError(responseWriter, request, err)
		return
	}
	if !found {
		jsonErr := map[string]string{"error": "api key not found"}
		httpx.RespondWithJSON(responseWriter, http.StatusUnauthorized, jsonErr)
		return
	}
	if findApiKey.Scope != "orders:read" {
		jsonErr := map[string]string{"error": "key lacks permissions for this scope"}
		httpx.RespondWithJSON(responseWriter, http.StatusForbidden, jsonErr)
		return
	}
