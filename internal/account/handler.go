...
func (handler *Handler) Page(responseWriter http.ResponseWriter, request *http.Request) {
	...
	formattedExpAt := formatTimestamp(current.Session.ExpiresAt)
	fields := map[string]any{
		"userId":    current.Session.UserID,
		"email":     current.User.Email,
		"expiresAt": formattedExpAt,
	}

	handler.logger.Event("account_accessed", fields)
...
func (handler *Handler) UpdateEmail(responseWriter http.ResponseWriter, request *http.Request) {
	...
	password, passErr := httpx.FormValue(request, "currentPassword")
	if passErr != nil {
	...
	passVerified := passwords.Verify(password, current.User.PasswordHash)
	if !passVerified {
		handler.errorPage(responseWriter, http.StatusForbidden, "", "Re-enter your current password to change your email.")
		return
	}
